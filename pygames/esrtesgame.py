...
#Animation eingefügt und mit GIMP überarbeitet
...
run = True
while run:
    clock.tick(32) #Gibt die Framerate bzw. Bildwiederholung an
    ...
    if not (isJump):
        #if keys[pygame.K_UP] and y  >  vel:  #Hoch laufen
          #  y -= vel
        
       # if keys[pygame.K_DOWN] and y < screenWitdh - height - vel: #runter laufen
          #  y += vel
        
        if keys[pygame.K_SPACE]:   # spring funktion
            isJump = True
            right = False
            left = False
            walkcount = 0
