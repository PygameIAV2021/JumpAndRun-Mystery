...
#BILDSCHIRM GRÖßEN ------------------------------------------------------------------------------------
pygame.display.set_caption('ZeroTwo') # set the window name
...
player_death = pygame.image.load('Grafiken/tot.png')
...
#Liest die map aus txt-Datei aus
def load_map(path):
    f = open(path + '.txt','r')
    data = f.read()
    f.close()
...
def Kollision_gegner():
    ...
    gegner2Rect= pygame.Rect(gegner2.x - player_rect.x - 116 , 170 - gegner2.y, gegner2.breite, gegner2.height) # player_rect von gegnerrect abziehen und minus extra wert damit die pixel abgestimtm sind
    
    #print(gegner2Rect,player_rect) für Debugg
...
gegner1= Gegner(400,355,4,13,17,[0,0],300,600) #x, y, geschw, breite,hoehe,richtg,xMin,xMax  Gegner wird erzeugt
gegner2= Gegner(400,139,2,13,17,[0,0],300,400) #x, y, geschw, breite,hoehe,richtg,xMin,xMax   Gegner wird erzeugt
flageRect= pygame.Rect(278,-4,17,23)
verloren = False
gewonnen = False
...
g = Game()  #Klasse von import game.py
g.running = True
while g.running:
